...
`lib/battle/stats.ts`(`STAT_RULESET` 2)가 큐레이션 카드 하나를 전투 스탯으로 바꿉니다. 같은 카드는
언제나 같은 스탯을 냅니다.

| 스탯 | 식 | 현재 매니페스트 범위 |
| --- | --- | --- |
| `atk` | `round(card.attack × 0.7)` | 약 70% |
| `def` | `round(card.defense × 0.7)` | 약 70% |
| `maxHp` | `40 + round(defense × 0.62) + 등급 보너스` | 낮춘 기본값 |
...
같은 카드 중복은 강화에 소모합니다. 0→1은 1장, 이후 2·3·4·5장. 최대 +5이며 단계마다 체력/공격/방어 8%, 치명타는 2단계마다 +1입니다. 전투 시작 때 강화 단계를 덱 스냅샷에 고정합니다.
