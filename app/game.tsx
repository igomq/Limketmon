...
function CollectionApp({ user, cards, initial }: { user: User; cards: Card[]; initial: Snapshot }) {
  ...
  return (
    <div className="app-shell">
      ...
      <footer className="footer"><span>limketmon. <span>조금 특별한, 우리의 순간들.</span></span><span>ORIGINAL COLLECTION · {cards.length} CARDS</span></footer>
...
function HomeView({ user, snapshot, cards, onNavigate, onOpen }: { user: User; snapshot: Snapshot; cards: Card[]; onNavigate: (tab: Tab) => void; onOpen: (card: Card) => void }) {
  ...
  return <>
    <section className="discovery-hero" aria-labelledby="discovery-title">
      <div className="hero-copy"><p className="eyebrow"><span className="live-dot" />THE ORIGINAL COLLECTION</p>
        <h1 id="discovery-title">평범한 순간,<br /><span>범상치 않은 카드.</span></h1>
        <p className="hero-description">세상에 하나뿐인 임신규 컬렉션.<br />한 장씩 발견하는 즐거움을 모아보세요.</p>
        ...
        <p className="hero-note"><Icon name="sparkle" />매일 한 장 무료 · {cards.length}종의 서로 다른 순간</p>
      </div>
    ...
    <section className="collection-invite"><div><span className="eyebrow">EVERY CARD TELLS A STORY</span><h2>다음 빈칸의 주인공은?</h2><p>{user ? `${cards.length - snapshot.inventory.length}개의 순간이 아직 기다리고 있어요.` : 'N부터 UR까지. 취향도, 희귀도도 제각각.'}</p></div><button className="btn btn-dark" onClick={() => onNavigate(user ? 'pull' : 'collection')}>{user ? '한 장 더 만나보기' : `${cards.length}개의 순간 보기`}<Icon name="arrow" /></button></section>
  </>;
}
...
function PullView({ user, snapshot, count, onCount, busy, results, pullId, onPull, onOpen, onNavigate }: { user: User; snapshot: Snapshot; count: 1 | 5; onCount: (count: 1 | 5) => void; busy: boolean; results: PullResult[]; pullId: number; onPull: () => void; onOpen: (card: Card) => void; onNavigate: (tab: Tab) => void }) {
  ...
  return <section className="pull-view" aria-labelledby="pull-title">
    <div className="section-head"><div><p className="eyebrow">A LITTLE SURPRISE, EVERY DAY</p><h1 id="pull-title">오늘의 발견.</h1></div><span className="edition-label">ORIGINALS / VOL. 01</span></div>
    <div className="pull-layout"><div className={`opening-table ${results.length ? 'has-results' : ''}`}>
      <div className="table-label"><Icon name="sparkle" />{results.length ? 'YOUR NEW MOMENTS' : 'SOMETHING GOOD INSIDE'}</div>
      ...
      <p className="table-hint" role="status">{busy ? <><span className="loading-dot" />카드를 가져오고 있어요…</> : results.length ? `${results.length}장 획득 · ${newCount ? `새로운 카드 ${newCount}장` : '컬렉션에 수량이 추가됐어요'}` : <><Icon name="hand" />팩을 누르면 새로운 순간이 열려요</>}</p>
    </div><aside className="pack-options"><div className="available-label"><span className="live-dot" />{user && snapshot.freeAvailable ? '오늘의 무료 팩 준비 완료' : '매일 한 장, 무료로'}</div><h2>어떤 순간을<br />만나게 될까요?</h2><p>평범해서 더 특별한 사진들.<br />팩을 열어 나만의 컬렉션에 더해보세요.</p>
      <div className="pack-selector" aria-label="뽑기 수량">{([1, 5] as const).map((value) => <button key={value} aria-pressed={count === value} disabled={busy} onClick={() => onCount(value)}>{count === value && <motion.span className="pack-selection" layoutId="pack-count" />}<span><strong>{value === 1 ? '한 장 발견' : '다섯 장 발견'}</strong><small>{value === 1 && snapshot.freeAvailable && user ? '오늘 1회 무료' : `뽑기권 ${value}장`}</small></span>{value === count && <Icon name="check" />}</button>)}</div>
...
function RevealDeck({ results, onOpen }: { results: PullResult[]; onOpen: (card: Card) => void }) {
  ...
  </div><div className="reveal-caption" aria-live="polite"><strong>{faceUp ? cardTitle(result.card) : '어떤 카드일까요?'}</strong><span>{faceUp ? `${result.card.rarity} · ${result.isNew ? '처음 만난 순간' : `${result.quantity}장째 수집`}` : '카드를 눌러 뒤집어보세요'}</span></div>
...
function CollectionView({ user, snapshot, cards, filter, onFilter, onOpen, onNavigate }: { user: User; snapshot: Snapshot; cards: Card[]; filter: CollectionFilter; onFilter: (filter: CollectionFilter) => void; onOpen: (card: Card) => void; onNavigate: (tab: Tab) => void }) {
  ...
  return <section className="collection-view" aria-labelledby="collection-title">
    <header className="section-head"><div><p className="eyebrow">THE COMPLETE ARCHIVE</p><h1 id="collection-title">순간을 모으는 도감.</h1><p>{user ? '하나씩 채워가는, 나만의 임신규 컬렉션.' : `${cards.length}가지의 임신규. 마음에 드는 카드를 먼저 만나보세요.`}</p></div><div className="completion-number"><strong>{user ? snapshot.inventory.length : cards.length}</strong><span>/ {cards.length}<small>{user ? '발견한 순간' : '전체 카드'}</small></span></div></header>
...
function CouponView({ user, busy, feedback, onSubmit, onNavigate }: { user: User; busy: boolean; feedback: Feedback; onSubmit: (event: FormEvent<HTMLFormElement>) => void; onNavigate: (tab: Tab) => void }) {
  return <section className="coupon-view" aria-labelledby="coupon-title"><div className="section-head"><div><p className="eyebrow">MORE MOMENTS TO COLLECT</p><h1 id="coupon-title">반가운 선물이 도착했어요.</h1><p>쿠폰 한 장으로, 더 많은 발견을.</p></div></div><div className="coupon-layout"><div className="welcome-ticket"><span className="eyebrow">A GIFT FOR YOUR COLLECTION</span><div className="ticket-value">100<span>장의<br />새로운 기회</span></div><p>첫 수집을 응원하는 웰컴 쿠폰.</p><div className="ticket-code"><span>WELCOME CODE</span><strong>LIMKETMON</strong><Icon name="ticket" /></div><span className="ticket-footnote">계정당 1회 · 뽑기권 100장</span></div><div className="coupon-form-panel"><Icon name="ticket" /><h2>컬렉션에 기회를 더하세요.</h2><p>가지고 있는 쿠폰 코드를 입력해주세요.</p>{user ? <form onSubmit={onSubmit}><label htmlFor="coupon-code">쿠폰 코드</label><input id="coupon-code" name="code" placeholder="LIMKETMON" maxLength={64} required autoComplete="off" autoCapitalize="characters" spellCheck={false} /><button className="btn btn-primary" disabled={busy}>{busy ? '쿠폰 확인 중…' : '뽑기권 받기'}<Icon name="arrow" /></button></form> : <a className="btn btn-primary" href={signIn('coupon')}>로그인하고 선물 받기<Icon name="arrow" /></a>}{feedback && <div className={`feedback ${feedback.error ? 'error' : 'success'}`} role={feedback.error ? 'alert' : 'status'}><Icon name={feedback.error ? 'clock' : 'check'} /><span>{feedback.text}{!feedback.error && <button className="text-button" onClick={() => onNavigate('pull')}>카드팩 열러 가기<Icon name="arrow" /></button>}</span></div>}<p className="coupon-note">받은 뽑기권은 내 계정에 저장됩니다.<br />이미 사용한 쿠폰은 다시 사용할 수 없어요.</p></div></div></section>;
}
