...
- 속성 링은 `light → shadow → nature → iron → spark → light`이며 다음 속성에게 +25%, 이전 속성에게 −20%가 적용됩니다.
...
- 첫 격파는 상대별 1회만 지급합니다(위 PvE 표의 2/3/4/5/8). 같은 상대를 반복 격파해도 0입니다.
- 데일리 승리는 하루 1회 3, 업적은 최초 달성 시 1회 지급합니다.
- 모든 지급은 `reward_claims`(기본키 `user_id` + `claim_key`)로 잠그고 D1 batch 한 트랜잭션에서 처리하므로, 중간에 실패하면 지급이 통째로 취소됩니다.
...
- SSR 이상이 나오지 않은 뽑기가 30회 쌓이면 다음 뽑기부터 확률이 오릅니다. 회차마다 0.02씩(최대 0.55) SSR·UR 가중치로 옮기고 N·R·SR에서 같은 만큼 덜어내므로 표 합은 항상 1입니다.
- 60회째는 하드 천장으로 SSR 이상이 확정입니다(UR 22% / SSR 78%).
- SSR 이상이 나오면 카운터가 0으로 돌아갑니다. 저장이 실패하면 차감·지급과 함께 천장 카운터도 되돌립니다.
...
- 천장 카운터는 뽑기 트랜잭션 안에서 저장된 pull_history 로부터 다시 계산합니다.
- 동시에 들어온 뽑기가 같은 카운터를 읽어 하드 천장을 여러 번 받는 일은, 카운터가 그대로일 때만 통과하는 비교-교체와 trg_user_game_state_pity_guard 트리거가 막습니다. 롤백되는 요청은 아무것도 지급하지 않습니다.
...
- `user_game_state`: 뽑기권, 마지막 무료 뽑기 KST 날짜, 뽑기 천장 카운터
...
- `pnpm test`: KST 날짜·희귀도 확률과 천장, 이미지·metadata, 도감 검색·필터·정렬, 제스처 도착 위치 계산, SQLite 기반 뽑기 차감·지급의 원자성을 검사합니다.
...
- `tests/battle-server.test.ts`: 덱 소유 검증, 서버 재시뮬레이션, 1회성 보상과 동시 종료, 데일리, 천장 롤백을 실제 migration 위에서 검사합니다.
- `tests/progression.test.ts`: 덱 규칙, 데일리 규칙 회전, 업적 8종, 보상 계획, 통계 집계를 검사합니다.
- tests/battle-races.test.ts: 병렬 뽑기의 천장 파밍, 스타터 덱 경쟁, 덱 상한, 데일리 뱅킹, pending 상한, 로그 없는 정산의 재시도 가능성을 검사합니다.
