...
export function combatantSeed(cardId: string, hpScale = 1): CombatantSeed {
...
export function playerTeam(cardIds: string[]): CombatantSeed[] {
  return cardIds.map((cardId) => combatantSeed(cardId, 1));
}
...
export function buildSetup(options: {
  ...
  return {
    ...
    seed: options.seed >>> 0,
    player: playerTeam(options.playerCardIds),
