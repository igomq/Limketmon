...
export async function pullCard(userId: string): Promise<{
  card: Card;
  isNew: boolean;
  quantity: number;
  ...
  const usedFreePull = (freeClaim.meta.changes ?? 0) > 0;
  ...
  const card = pickCard(rollRarity(randomUnit()));
  const pulledAt = now.toISOString();
  const [inventoryResult] = await db.batch([
    db.prepare(`
  ...
  const quantity = Number((inventoryResult.results[0] as { quantity: number }).quantity);
